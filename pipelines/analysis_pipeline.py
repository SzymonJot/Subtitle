...
from domain.nlp.lexicon.schema import AnalyzedEpisode, LemmaSV, Stats
...
def process_episode(
    file_bytes, adapter: ContentAdapter, lang_adapter: LangAdapter
) -> bytes:
    t0 = _t()
    ...
    # 4) lemmas
    t3 = _t()
    lexicon: Dict[str, LemmaSV] = lang_adapter.build_dictionary_from_tokens(
        tokens, words_counted
    )
