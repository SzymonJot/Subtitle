...
class SBJobsIO:
    ...
    def insert_job(
        self, job_id: str, in_path: str, jobs_table: str, status: str, params: dict
    ):
    ...
    def download_analysis(self, job_id: str) -> Any:
        ...
        file_path = "/".join(
            path_str.split("/")[1:]
        )  # Handle paths with multiple slashes if any
    ...
    def get_cached(self, ids: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        if not ids:
            return {}
        res = self.sb.table(TABLE_CACHE).select("*").in_("id", ids).execute()
    ...
    def upsert_cache_translation(self, entries: List[Dict]) -> Any:
        ...
        if not entries:
            return {}
        res = self.sb.table(TABLE_CACHE).upsert(entries).execute()
        return res
